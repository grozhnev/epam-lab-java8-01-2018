...
public class Exercise1 {
    ...
    public void calcTotalYearsSpentInEpam() {
        ...
        Long hours = employees.stream()
                .flatMap(e -> e.getJobHistory().stream().filter(p -> "EPAM".equals(p.getEmployer())))
    ...
    public void findPersonsWithQaExperience() {
        ...
        Set<Person> workedAsQa =
                employees.stream()
                        .filter(e -> e.getJobHistory().stream().map(JobHistoryEntry::getPosition)
                                .anyMatch("QA"::equals))
    ...
    public void groupPersonsByFirstPositionUsingToMap() {
        ...
        Map<String, Set<Person>> result = employees.stream().collect(Collectors.toMap(
                e -> e.getJobHistory().get(0).getPosition(),
                e -> {
                    Set<Person> objects = new HashSet<>();
                    objects.add(e.getPerson());
                    return objects;
                },
                (v1, v2) -> {
                    v1.addAll(v2);
        ...
        Map<String, Set<Person>> result =
                employees.stream()
                        .collect(Collectors.groupingBy(
                                e -> e.getJobHistory().get(0).getPosition(), Collectors.mapping(Employee::getPerson, Collectors.toSet())));
